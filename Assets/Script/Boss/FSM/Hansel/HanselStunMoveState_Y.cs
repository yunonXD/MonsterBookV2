...
public class HanselStunMoveState_Y : FSM_State<Hansel>
{
    private Vector3 m_OldPosition;
    private Vector3 m_CurrentPosition;
    private float m_SpeedBoi = 0;

    static readonly HanselStunMoveState_Y instance = new HanselStunMoveState_Y();
    public static HanselStunMoveState_Y Instance { get { return instance; } }
    ...
    public override void EnterState(Hansel _Hansel)
    {
        ...
        //_Hansel.rb.constraints = RigidbodyConstraints.None;
        _Hansel.rb.constraints -= 8;

        _Hansel.CapCol_Hansel.GetComponent<CapsuleCollider>().isTrigger = true;
    ...
    {
        _Hansel.CapCol_Hansel.GetComponent<CapsuleCollider>().isTrigger = false;
    ...
    public override void UpdateState(Hansel _Hansel)
    {
        if (!_Hansel.isSmash && !_Hansel.isBelly &&!_Hansel.isRolling &&!_Hansel.isRushing )
        {
            ...
            var m_lookatVec = (new Vector3(_Hansel.RuntoGretelPosition_Y.transform.position.x, 0, 0)
            ...
            #region Movement
            var m_Dir = new Vector3(_Hansel.RuntoGretelPosition_Y.transform.position.x - _Hansel.transform.position.x, 0, _Hansel.RuntoGretelPosition_Y.transform.position.z - _Hansel.transform.position.z).normalized;
